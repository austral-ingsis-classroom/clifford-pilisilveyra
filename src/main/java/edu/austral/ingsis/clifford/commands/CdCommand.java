...
public record CdCommand(String directoryInfo) implements Command {
  ...
  public String executeCommand(FileSystemSession session) {
    List<String> resolvedPath = PathResolver.resolve(session.getCurrentPathSegments(), directoryInfo);
    Optional<Directory> maybeDirectory = DirectoryNavigator.navigate(session.getRoot(), resolvedPath);
